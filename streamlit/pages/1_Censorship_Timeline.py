...
st.caption(
    "OONI blocking measurements over time — daily trends, monthly heatmap, and status breakdown")
...
if granularity == "Daily":
    agg = daily_df.groupby(["measurement_date", "censorship_status"])[
        "blocked"].sum().reset_index()
    fig = px.area(
        agg, x="measurement_date", y="blocked",
        color="censorship_status",
        color_discrete_map=STATUS_COLORS,
        labels={"measurement_date": "", "blocked": "Blocked measurements",
                "censorship_status": "Status"},
        height=340,
    )
elif granularity == "Weekly":
    agg = daily_df.copy()
    agg["week"] = pd.to_datetime(
        agg["measurement_date"]).dt.to_period("W").dt.start_time
    agg = agg.groupby(["week", "censorship_status"])[
        "blocked"].sum().reset_index()
    fig = px.bar(
        agg, x="week", y="blocked",
        color="censorship_status",
        color_discrete_map=STATUS_COLORS,
        labels={"week": "", "blocked": "Blocked",
                "censorship_status": "Status"},
        height=340,
    )
else:
    agg = daily_df.copy()
    agg["month"] = pd.to_datetime(
        agg["measurement_date"]).dt.to_period("M").dt.start_time
    agg = agg.groupby(["month", "censorship_status"])[
        "blocked"].sum().reset_index()
    fig = px.bar(
        agg, x="month", y="blocked",
        color="censorship_status",
        color_discrete_map=STATUS_COLORS,
        labels={"month": "", "blocked": "Blocked",
                "censorship_status": "Status"},
        height=340,
        barmode="stack",
    )
...
st.caption(
    "Each cell = % of measurements blocked that month. Darker red = higher censorship.")

pivot = monthly_df.pivot(
    index="year", columns="month_name", values="blocking_rate")
...
fig_heat = go.Figure(go.Heatmap(
    ...
    showscale=True,
    colorbar=dict(title="Block %", tickfont=dict(
        color="#E8E6DF"), titlefont=dict(color="#E8E6DF")),
    hoverongaps=False,
))
...
dow_labels = {1: "Sun", 2: "Mon", 3: "Tue",
              4: "Wed", 5: "Thu", 6: "Fri", 7: "Sat"}
calendar_df["dow_label"] = calendar_df["dow"].map(dow_labels)
calendar_df["month_period"] = pd.to_datetime(
    calendar_df["measurement_date"]).dt.to_period("M").astype(str)

pivot2 = calendar_df.groupby(["month_period", "dow_label"])[
    "blocking_rate"].mean().reset_index()
pivot2 = pivot2.pivot(
    index="dow_label", columns="month_period", values="blocking_rate")
