...
* OONI blocking signals (normalized to platform)
* Google Transparency + Lumen takedowns
...
SELECT
    ...
    CASE
        ...
        WHEN LOWER(input) LIKE '%twitter%' OR LOWER(input) LIKE '%x.com%' THEN 'twitter'
...
SELECT
    ...
    CASE
        ...
        WHEN LOWER(platform) LIKE '%twitter%' OR LOWER(platform) LIKE '%x%' THEN 'twitter'
