...
/* =========================
   LUMEN (clean — no more microsecond hack)
   ========================= */
lumen_spine AS (
    SELECT
        measurement_date,
...
SELECT
    *,
    ...
    CASE
        WHEN block_rate > 0.7 AND conflict_events > 0 AND takedown_requests > 0 
            THEN 'High Multi-Source Suppression'
        WHEN block_rate > 0.7 AND conflict_events > 0 
            THEN 'Conflict-Aligned Blocking'
        WHEN block_rate > 0.7 
            THEN 'Network Blocking Only'
        WHEN conflict_events > 0 
            THEN 'Conflict Pressure Only'
        WHEN takedown_requests > 0 
            THEN 'Legal / Platform Pressure Only'
        ELSE 'Baseline'
