...
description: |
  NEW — Monthly aggregated takedown request volumes by source and reason group.
  Grain: one row per (source, reason_group, year_month).
  Pre-aggregated to power the Streamlit "Content Removal Trends" chart
  without scanning the full row-level fact_takedown_requests table.
...
WITH base AS (
    SELECT
        ...
        platform,
        reason,
        -- Replicate the reason grouping logic here since dim_reasons
        -- is a lookup and we want this fact to be self-contained
        CASE
...
SELECT
    ...
    -- Month-over-month context (available in Streamlit via windowing
    -- on this pre-aggregated table — much cheaper than on row-level fact)
