...
description: |
  ...
  v3: effective baseline stddev (30d/90d sample with floor); NULL-safe
  GREATEST for BigQuery; zero_variance_flag only for flat-empirical + deviating day.
...
feature_thresholds AS (
  SELECT
    ...
    1e-9 AS rate_equality_tolerance
),
...
with_effective_variance AS (
  SELECT
    b.*,
    GREATEST(
      COALESCE(NULLIF(b.baseline_signal_stddev_30d, 0), NULLIF(b.baseline_signal_stddev_90d, 0), 0),
      t.baseline_stddev_floor
    ) AS baseline_signal_stddev_effective
  FROM with_baselines AS b
  CROSS JOIN feature_thresholds AS t
)
...
SELECT
  ...
  (
    (COALESCE(baseline_signal_stddev_30d, 0) = 0 OR baseline_signal_stddev_30d IS NULL)
    AND (COALESCE(baseline_signal_stddev_90d, 0) = 0 OR baseline_signal_stddev_90d IS NULL)
    AND ABS(signal_rate - COALESCE(baseline_signal_rate_30d, signal_rate)) > rate_equality_tolerance
  ...
  'protocol_daily_signals_v3' AS feature_version,
